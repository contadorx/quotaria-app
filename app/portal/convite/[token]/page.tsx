...
import { aceitarConvite, entrarPortal, criarContaPortal, sairPortal } from '../../actions'

export const dynamic = 'force-dynamic'

export default async function ConvitePortal({
  params,
  searchParams,
}: {
  params: { token: string }
      ...
      <div className="w-full max-w-md">
        <div className="mb-6 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-ink-muted">Portal da família</p>
          </div>
        </div>

        <div className="rounded-xl2 border border-line bg-white p-6 shadow-card">
          <h1 className="text-lg font-bold text-ink">Você foi convidado a acompanhar sua família</h1>
          <p className="mt-1 text-sm text-ink-muted">
            Acesso em modo leitura à estrutura, ao que foi feito e ao andamento da sucessão — preparado pelo seu escritório.
          </p>
          ...
          {user ? (
            <div className="mt-5 space-y-3">
              ...
              <form>
                <PendingButton action={sairPortal} className="w-full rounded-lg border border-line px-4 py-2 text-sm font-medium text-ink-muted transition hover:bg-surface">
                  Não sou eu — sair
                </PendingButton>
              </form>
            </div>
            ...
            <form className="mt-5 space-y-4">
              ...
              <div className="flex gap-3">
                <PendingButton action={entrarPortal} className="flex-1 rounded-lg bg-navy px-4 py-2 text-sm font-semibold text-white transition hover:bg-navy-soft">
                  Entrar
                </PendingButton>
                <PendingButton action={criarContaPortal} className="flex-1 rounded-lg border border-line px-4 py-2 text-sm font-semibold text-ink transition hover:bg-surface">
                  Criar conta
                </PendingButton>
              </div>
