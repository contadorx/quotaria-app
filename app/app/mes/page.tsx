...
export default async function MesPage({
  ...
  const mes = Number(searchParams.mes) || hoje.getMonth() + 1 // 1-12
  ...
  const { data: fechs } = await supabase
    .from('fechamentos')
    .select('holding_id, distribuicoes_ok, documentos_ok, alertas_ok, alugueis_ok, notes')
  ...
  return (
    <div>
      <PageHeader
        ...
        description="O fechamento guiado de cada holding — 10 minutos que mantêm a proteção viva e viram o extrato de tranquilidade da família."
      />
        ...
        <div className="text-center">
          ...
          {(holdings ?? []).length > 0 && (
            <div className="mt-0.5 text-xs text-ink-soft">
              {emDia} em dia · {pend} com pendências · {naoIniciado} a iniciar
            </div>
          )}
        </div>
              ...
              <EditDialog title={`Fechamento · ${MESES[mes - 1]}/${ano}`} label={f ? 'Revisar' : 'Fechar mês'}>
                <form className="space-y-4">
                    ...
                    {ITENS.map((it) => (
                      <label key={it.campo} className="flex items-start gap-2.5 text-sm text-ink">
                        <input
                          type="checkbox"
                          name={it.campo}
                          defaultChecked={f ? (f[it.campo as keyof Fechamento] as boolean) : false}
                  ...
                  <div className="flex justify-end">
                    <SubmitButton action={salvarFechamento}>Salvar fechamento</SubmitButton>
                  </div>
                </form>
              </EditDialog>
            </div>
